...
class SegD(SeismicReader):
    ...
    def read(self) -> None:
        """Read the file. (Already done in init for this implementation)"""
        if not self.data.parsed_data:
            self.data.read()

    @property
    def traces(self):
        return self.data.get_traces()
    ...
    def sample_rate(self) -> float:
        """Get sample rate in microseconds."""
        # This might need to be extracted from headers more reliably
        # For now returning a default or extracting from first trace extension if available
        if self.traces:
             # Assuming ext1 has sample rate, need to verify with specific format
             # This is a placeholder as the original code didn't expose it clearly
             return 2000.0 
        return 0.0
    ...
    def get_trace_data(self, index: int) -> np.ndarray:
        ...
        if 0 <= index < self.num_traces:
            return np.array(self.traces[index].trace_data)
    ...
    def get_trace_header(self, index: int) -> Dict[str, Any]:
        ...
        if 0 <= index < self.num_traces:
            # Combine demux header and extensions for a full header view
            header = dict(self.traces[index].demux_header)
            # We could add extensions here too
            return header
        raise IndexError(f"Trace index {index} out of range")
    ...
    def convert_to_seisdata(self, schema_file: str = None) -> 'SeisData':
        ...
        """
        from pyseis_io.seisdata.seisdata import SeisData
        import os
    ...
    def save_to_hdf5(self, output_file: str, schema_file: str = None) -> None:
        ...
        """
        import pandas as pd # Import pandas locally as it was missing in original imports but used here
